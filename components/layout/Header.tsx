...
import { Menu, X, Search, Heart, LayoutGrid, Info, Users, ArrowRight } from 'lucide-react';
...
export default function Header() {
  ...
  // Efek scroll untuk mengubah tampilan header
  useEffect(() => {
  ...
  const navLinks = [
    { name: 'Market', href: '/id/market', icon: Search },
    { name: 'Cara Kerja', href: '/id/faq', icon: LayoutGrid },
    { name: 'Donasi', href: '/id/donasi', icon: Heart, badge: 'Baru' },
    { name: 'Komunitas', href: '/id/komunitas', icon: Users },
  ];
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-8 flex justify-between items-center">
        ...
        {/* Brand/Logo */}
        <Link href="/id" className="group flex items-center gap-2.5">
        ...
        <nav className="hidden lg:flex items-center gap-8">
          ...
          <div className="flex items-center gap-3">
            <Link key={'login'} href="/id/login" className="px-5 py-2.5 text-sm font-bold text-gray-700 hover:text-green-600 transition-colors">
              Masuk
            </Link>
            <Link 
            key={'register'}
              ...
              <div className="flex flex-col gap-6">
                {navLinks.map((link) => (
                  ...
                  >
                    ...
                    {link.badge && <span className="px-2 py-0.5 bg-green-500 text-[10px] text-white rounded-full uppercase tracking-widest">{link.badge}</span>}
                  </Link>
                ))}
              </div>

              <div className="absolute bottom-10 left-8 right-8 space-y-4">
                <Link href="/login" className="block w-full text-center py-4 font-bold text-gray-700 bg-gray-50 rounded-2xl transition-colors">
                  Masuk
                </Link>
                <Link href="/register" className="block w-full text-center py-4 font-bold text-white bg-green-600 rounded-2xl shadow-lg shadow-green-100 transition-transform active:scale-95">
                  Daftar Sekarang
                </Link>
              </div>
            </motion.div>
